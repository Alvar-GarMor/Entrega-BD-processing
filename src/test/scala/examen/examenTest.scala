...
class examenTest extends TestInit {
  ...
  "Ejercicio 5" should "calcula el ingreso total" in {
    ...
    val ventas = ejercicio5("C:\\Users\\Usuario\\OneDrive\\Documentos\\Escritorio\\Keepcoding\\Procesamiento Big Data\\bd-processing-main\\bd-processing-main\\src\\main\\scala\\examen_estructura\\ventas.csv")
    // mostrar schema y datos:
    ventas.printSchema()
    ventas.show(false)


    val out = ventas.select("id_producto", "ingreso total")
      .as[(Int, Double)] // Convierto en Dataset para que se pueda aplicar .toList
      .collect()
    /*   Descomentar el assert siguiente para comprobar el ejercicio */
